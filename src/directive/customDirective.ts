...
/**
 * 节流指令.
 * @desc 首次点击直接触发。连续点击时，每指定时间触发一次
 * @param {App} app - The Vue 3 app instance
 * @default time 1000ms
 * @use v-throttle:number = "clickEvent"
 */
export function throttleDirective(app: App) {
  app.directive("throttle", {
    beforeMount(el: HTMLElement, binding: { arg?: string; value: (...args: any[]) => any }) {
      el.onclick = throttle(binding.value, Number(binding.arg) || 1000); // 默认1秒节流时间
    },
  });
}
/**
 ...
 * @param {App} app - The Vue 3 app instance
 * @default time 1000ms
 * @use v-debounce:number = "clickEvent"
 */
...
export function throttle<T>(func: (...args: T[]) => any, delay: number, leading: boolean = true): (...args: T[]) => void {
  let timeoutId: any;
  let lastExecTime = 0;
  return function (this: any, ...args: T[]) {
    const context = this;
    const currentTime = Date.now();
    const delaySinceLastExec = currentTime - lastExecTime;
    ...
    if (leading) {
      if (delaySinceLastExec >= delay) {
        execute();
      } else {
        clearTimeout(timeoutId);
        timeoutId = setTimeout(execute, delay - delaySinceLastExec); //重新创建定时器
      }
    } else {
